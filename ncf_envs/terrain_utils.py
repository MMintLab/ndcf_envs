...
def new_sub_terrain(tc):
    return SubTerrain(width=tc.num_rows, length=tc.num_cols, vertical_scale=tc.vertical_scale,
                      horizontal_scale=tc.horizontal_scale)
...
def generate_discrete_obstacles_terrain(tc):
    ...
    vertices, triangles = convert_heightfield_to_trimesh(tc.heightfield[: w // 3, : h // 3],
                                                         horizontal_scale=tc.horizontal_scale,
